...
//  int   n_rocs     = iConfig.getParameter<int>("n_rocs");
  ...
  addExtractor(intern("ROCinLayerRow"),
    [pxladder, pxlayer, maxladders, roc_rows] (InterestingQuantities const& iq) {
  ...
  addExtractor(intern("ROCinLayerCol"),
    [pxmodule, maxmodule, roc_cols] (InterestingQuantities const& iq) {
  ...
  addExtractor(intern("ROCinDiskRow"),
    [pxendcap, pxpanel, pxblade, innerring, roc_rows] (InterestingQuantities const& iq) {
      auto ec = pxendcap(iq);
  ...
  addExtractor(intern("ROCinDiskCol"),
    [pxendcap, pxblade, roc_cols] (InterestingQuantities const& iq) {
      auto ec = pxendcap(iq);
      if (ec == UNDEFINED) return UNDEFINED;
      auto blade = pxblade(iq);
      if (blade == UNDEFINED) return UNDEFINED;
