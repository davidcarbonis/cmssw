...
#include "Validation/SiPixelRecHitsV/interface/SiPixelRecHitsV.h"
...
SiPixelRecHitsV::SiPixelRecHitsV(const edm::ParameterSet& iConfig) :
  SiPixelPhase1Base(iConfig),
  trackerHitAssociatorConfig_( iConfig, consumesCollector() ),
  srcToken_ ( consumes<SiPixelRecHitCollection>(iConfig.getParameter<edm::InputTag>("src")) )
{}

void SiPixelRecHitsV::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
DEFINE_FWK_MODULE(SiPixelRecHitsV);
