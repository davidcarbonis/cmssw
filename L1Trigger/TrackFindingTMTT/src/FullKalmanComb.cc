...
void FullKalmanComb::printTP( std::ostream &os, const TP *tp )const{
...
FullKalmanComb::FullKalmanComb(const Settings* settings, const uint nPar, const string &kfName, const uint nMeas ) : TrackFitGeneric(settings, kfName ){
  nPar_ = nPar;
  nMeas_ = nMeas;
...
void FullKalmanComb::init(const Settings* settings, unsigned int iPhiSec, unsigned int iEtaReg, 
  ...
  // Check if subsectors are being used within each sector.
  ...
  // Full CKF Options
  seedingOption_           = settings_->kalmanSeedingOption_();
}

void FullKalmanComb::hitBuffer( const Stub* stub, const vector<bool>& inEtaSubSecs ) {
  ...
  if ( ( ! busyInputSectorKill_) || (nReceivedStubs_ <  busyInputSectorNumStubs_) ) {
    ...
    subSectors_[stub] = inEtaSubSecs;
    if (inSubSecs.size() != numSubSecs_) throw cms::Exception("FullKalmanComb: Wrong number of subsectors!");    
  }
...
void FullKalmanComb::createSeeds() {
  //=== Filter stubs
  // KF Bend Filter?
  // Sector/subsector compatible?

  // Is consistent with beamspot? i.e. z<15cm?

  vector<L1track3D> trackCands3D;
  const vector<const Stub*>& stubs = vStubs_;
  const pair<unsigned in, unsigned int> cellLocation { make_pair{0,0} }; // No HT seed location - use dummy location
	
  //=== Create Seeds  
  // Default option: L0 + beamspot
  if (seedingOption_ ==0) {
    // SPIT OUT A L1Track3D to be used by the KF fit
    ...
    trackCands3D.push_back()
  }
  // Something other seeding option?
}

L1fittedTrack FullKalmanComb::fit(const L1track3D& l1track3D){
...
std::vector<const kalmanState *> FullKalmanComb::doKF( const L1track3D& l1track3D, const std::vector<const StubCluster *> &stubClusters, const TP *tpa ){
...
const kalmanState *FullKalmanComb::kalmanUpdate( unsigned skipped, unsigned layer, const StubCluster *stubCluster, const kalmanState &state, const TP *tpa ){
...
double FullKalmanComb::calcChi2( const kalmanState &state )const{
...
double FullKalmanComb::Chi2( const TMatrixD &dcov, const std::vector<double> &delta, bool debug )const
{
  if( getSettings()->kalmanDebugLevel() >= 4 ){
    cout << "dcov" << endl;
    dcov.Print();
...
std::map<std::string, double> FullKalmanComb::getTrackParams( const FullKalmanComb *p, const kalmanState *state )
{
  return p->getTrackParams( state );
}


std::vector<double> FullKalmanComb::Hx( const TMatrixD &pH, const std::vector<double> &x )const
{
...
std::vector<double> FullKalmanComb::Fx( const TMatrixD &pF, const std::vector<double> &x )const
{
  return Hx( pF, x );
}


TMatrixD FullKalmanComb::HxxH( const TMatrixD &pH, const TMatrixD &xx )const
{
...
TMatrixD FullKalmanComb::GetKalmanMatrix( const TMatrixD &h, const TMatrixD &pxcov, const TMatrixD &dcov )const
{
...
void FullKalmanComb::GetAdjustedState( const TMatrixD &K, const TMatrixD &pxcov, 
...
void FullKalmanComb::resetStates()
{
...
const kalmanState *FullKalmanComb::mkState( const L1track3D &candidate, unsigned skipped, unsigned layer, unsigned layerId, const kalmanState *last_state, 
...
std::vector<double> FullKalmanComb::residual(const StubCluster* stubCluster, const std::vector<double> &x, double candQoverPt )const{
...
void FullKalmanComb::bookHists(){

  if ( getSettings()->kalmanFillInternalHists() ) {

    edm::Service<TFileService> fs_;
    string dirName;
    if( kfName_.compare("") == 0 ) dirName = "FullKalmanCombInternal";
    else dirName = kfName_ + "Internal";
...
void FullKalmanComb::fillCandHists( const kalmanState &state, const TP *tpa )
{
  if( tpa && tpa->useForAlgEff() ){
...
void FullKalmanComb::fillSeedHists( const kalmanState *state, const TP *tpa ){
...
void FullKalmanComb::fillStepHists( const TP *tpa, unsigned nItr, const kalmanState *new_state )
{
  unsigned path = 0;
...
void FullKalmanComb::deleteStubClusters()
{
...
double FullKalmanComb::DeltaRphiForClustering( unsigned layerId, unsigned endcapRing )
{
...
double FullKalmanComb::DeltaRForClustering( unsigned endcapRing )
{
...
bool FullKalmanComb::isOverlap( const Stub* a, const Stub*b, OVERLAP_TYPE type ){
...
set<unsigned> FullKalmanComb::getKalmanDeadLayers( unsigned layerMap[18][8], bool& remove2PSCut ) const {
...
float FullKalmanComb::getApproxB(float z, float r) const {
